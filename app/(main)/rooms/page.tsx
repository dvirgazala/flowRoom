...
import { getMyRooms, createRoom } from '@/lib/db'
import type { DbRoom } from '@/lib/supabase-types'
import { Plus, ChevronLeft, Loader2, Music2 } from 'lucide-react'
...
export default function RoomsPage() {
  ...
  const stageProgress = (room: DbRoom) => Math.round((room.current_stage / (STAGES.length - 1)) * 100)
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      {showNew && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-bg1 rounded-2xl p-6 w-full max-w-md shadow-surface-lg"
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-text-muted text-sm mb-6">
            צור חדר חדש והזמן שותפים — או קבל הזמנה מחבר לחדר שלו
          </p>
          <button onClick={() => setShowNew(true)}
              ...
              <Link key={room.id} href={`/rooms/${room.id}`}
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    ...
                    {room.genre && <p className="text-text-muted text-xs mt-0.5">{room.genre}</p>}
                  </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="h-1.5 bg-bg3 rounded-full overflow-hidden">
                    <div className="h-full bg-brand-gradient rounded-full transition-all"
                      style={{ width: `${stageProgress(room)}%` }} />
                  </div>
                </div>
