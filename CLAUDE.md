...
- Supabase (Auth only, all other state lives client-side)
...
2. **State Management:** All state mutations flow through Zustand actions in `lib/store.ts`. No `useState` for persisted data.
